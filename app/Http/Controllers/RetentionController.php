...
class RetentionController extends Controller
{
    public function getRetentionStores(Request $request, OdooService $odooService)
    {
        ...
        $cacheKey = 'retention:stores:' . hash(
            'sha256',
            basename($latestFile) . ':' . File::lastModified($latestFile)
        );

        $baseData = Cache::remember($cacheKey, now()->addMinutes(15), function () use ($latestFile, $odooService) {
            ...
            return $excelData->map(function (array $row) use ($odooStores) {
        ...
        $partnerIds = collect($baseData)->pluck('partner_id')->toArray();
        ...
        $blockingVisits = StoreVisit::with('sales')
            ...
            ->orderByRaw("CASE WHEN status = 'IN_VISIT' THEN 1 ELSE 0 END")
            ->orderBy('id')
            ->get()
            ->keyBy('odoo_partner_id');

        $processedData = collect($baseData)->map(function (array $row) use ($blockingVisits, $currentUserId) {
            $partnerId = (int) ($row['partner_id'] ?? 0);
            $activeVisit = $blockingVisits->get($partnerId);
